...
\begin{prob}
  Rozstrzygnij czy następujące zbiory z działaniami są grupami:
  \begin{enumerate}
    \item $(\mathbb Z, +)$
    \item $(\mathbb Z, \cdot)$
    \item $(\mathbb Q, \cdot)$
...
\begin{prob}
  [2] Pokaż, że jeśli $f: G\to H$ jest homomorfizmem, to $f(g^{-1})=f(g)^{-1}$ oraz $f(e)=\epsilon$, gdzie $e\in G, \epsilon\in H$ oznaczają elementy neutralne odpowiednich grup.
\end{prob}
...
\begin{prob}
  [7] Ile jest, z dokładnością do izomorfizmu, grup:
  \begin{enumerate}
